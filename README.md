...
자주 쓰는 원물/박스 품목은 [items.html](items.html)에서 등록·수정·비활성화할 수 있습니다.
[input.html](input.html)에서 입력하다가 "+ 새 품목 추가"로 그때그때 추가해도 됩니다.
...
- [items.html](items.html)에서 원물/박스 품목을 추가하거나 매입단가·판매단가를 수정합니다.
- [labor.html](labor.html)에서 이모님 등 일당제 근무자를 등록하고, 오늘 출근한 사람을 체크만 하면
  자동으로 근무기록이 쌓입니다. 월별 캘린더에서 근무자별 근무일수·지급액을 한눈에 확인할 수 있습니다.
- [index.html](index.html) 대시보드에서 오늘 매출/순이익(비용 항목별 내역 포함), 원물·박스 재고 현황,
  최근 30일 추이, 최근 폐기 내역을 확인합니다.

## 순이익 계산 방식
```
순이익 = 매출 − 원물출하원가 − 박스비 − 폐기손실 − 택배비 − 인건비 − 기타비용
```
- 원물출하원가/박스비: `출하`로 기록한 수량 × 품목 매입단가
